...
export default function VendorsPage() {
  ...
  const handleDeleteVendor = async (vendorId: string) => {
    ...
    try {
      const response = await fetch(`/api/manufacturing/vendors/${vendorId}`, {
        method: 'DELETE'
      });
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    {filteredVendors.map((vendor) => (
                      <tr key={vendor.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td className="px-3 lg:px-6 py-3 lg:py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${vendor.type === 'Material' ? 'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400' :
                ...
                {filteredVendors.map((vendor) => (
                  <div key={vendor.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-md border-l-4 border-primary p-4 hover:shadow-lg transition-shadow duration-200">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span className={`px-3 py-1.5 text-xs font-bold rounded-lg flex items-center gap-1 ${vendor.type === 'Material' ? 'bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/30' :
