...
export async function POST(request: Request) {
  try {
    ...
    let newExpense = null;
    const shouldCreateExpense = category !== 'Company Labor' ||
      (category === 'Company Labor' && Number(laborPaidAmount) > 0);

    if (shouldCreateExpense) {
      const safeDescription = description || note || (category === 'Company Labor' ? `Payment to ${employeeId}` : 'Expense');
