...
export async function POST(request: Request) {
    ...
    try {
        ...
        // Universal Payment Status Fallback: If paidFrom is provided but status is UNPAID/missing, set to PAID
        let finalPaymentStatus = paymentStatus;
        if (paidFrom && paidFrom !== 'UNPAID' && (!finalPaymentStatus || finalPaymentStatus === 'UNPAID')) {
            finalPaymentStatus = 'PAID';
        }
        const finalPaymentDate = reqBody.paymentDate || (finalPaymentStatus === 'PAID' ? (expenseDate || new Date()) : null);
        ...
        // 1. Create the expense (Always create for non-Labor or for any Labor payment)
        let newExpense = null;
        const shouldCreateExpense = category !== 'Labor' ||
            (category === 'Labor' && Number(laborPaidAmount) > 0);
        ...
        if (shouldCreateExpense) {
            ...
            const safeDescription = category === 'Utilities' ? (description || 'Utilities expense') :
                (description || note || (category === 'Transport' ? transportType : '') || (category === 'Labor' ? `Payment to ${employeeId}` : 'Expense'));
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
    try {
        ...
        if (existingLabor.paidFrom && existingLabor.paidFrom !== 'UNPAID' && Number(existingLabor.paidAmount) > 0) {
            // Check if it's a valid UUID/ID by attempting to find it or just trust but verify
            await prisma.account.update({
                where: { id: existingLabor.paidFrom },
                data: {
                    balance: { increment: Number(existingLabor.paidAmount) }, // Soo celi lacagta
                },
            }).catch(err => console.warn('Could not refund account during ProjectLabor delete:', err));
        }
