...
const EmployeeDetailsPage: React.FC = () => {
  ...
  const fetchEmployeeDetails = async () => {
    ...
    try {
      ...
      const employeeData = {
        ...data.employee,
        monthlySalary: parseFloat(data.employee.monthlySalary) || 0,
        totalPaid: parseFloat(data.employee.totalPaid) || 0,  // NEW: Actual total paid
        totalSalaryOwed: parseFloat(data.employee.totalSalaryOwed) || 0,  // NEW: Total owed
        totalRemaining: parseFloat(data.employee.totalRemaining) || 0,  // NEW: Remaining
        overpaidAmount: parseFloat(data.employee.overpaidAmount) || 0,
        totalMonthsWorked: parseInt(data.employee.totalMonthsWorked) || 0,  // NEW: Months worked
      ...
      if (employeeData.category === 'COMPANY') {
        // Values are already calculated by API, just use them directly
        // No need for frontend calculations anymore
        employeeData.dailyRate = employeeData.monthlySalary / 30;  // Approximate daily rate
      } else {
  ...
  // Subscribe to expense changes (real-time updates)
  useEffect(() => {
    if (!id) return;

    const unsubscribe = subscribeToExpenseChange((payload) => {
      // Refresh employee data when an expense related to this employee changes
      if (payload.employeeId === id || payload.action === 'create' || payload.action === 'update') {
        // Refresh data silently without showing toast
        fetchEmployeeDetails();
        fetchSalaryExpenses();
        fetchSalarySummary();
      }
        ...
        <div className="block md:hidden mb-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={refreshData}
              disabled={refreshing}
              className={`p-2 rounded-lg transition duration-200 ${refreshing
          ...
          <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md border-l-4 border-primary mb-4">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              <span className={`px-2 py-1 rounded-full text-xs font-bold ${employee.category === 'COMPANY'
        ...
        <div className="hidden md:flex justify-between items-center mb-8">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mt-2">
                <span className={`px-3 py-1 rounded-full text-sm font-bold ${employee.category === 'COMPANY'
                  ? 'bg-primary/10 text-primary border border-primary/30'
                  : 'bg-accent/10 text-accent border border-accent/30'
                  }`}>
          ...
          <div className="flex space-x-3">
            <button
              onClick={refreshData}
              disabled={refreshing}
              className={`py-2.5 px-6 rounded-lg font-bold text-lg transition duration-200 shadow-md flex items-center ${refreshing
                ? 'bg-gray-400 text-white cursor-not-allowed'
                : 'bg-primary text-white hover:bg-blue-700'
                }`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 pb-4 md:pb-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-4 md:p-5 rounded-xl shadow-md border-l-4 border-secondary hover:shadow-xl transition-all duration-300">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <div>
                  <h4 className="text-sm font-medium text-mediumGray dark:text-gray-400">Wadarta La Bixiyay</h4>
                </div>
              </div>
              <p className="text-2xl font-extrabold text-secondary">{employee.totalPaid?.toLocaleString() || 0} ETB</p>
              <p className="text-xs text-mediumGray dark:text-gray-400 mt-1">
                Wadarta Lacagtii Loo Bixiyay Dhan
              </p>
            </div>

            {/* Remaining This Month */}
            <div className={`bg-white dark:bg-gray-800 p-4 md:p-5 rounded-xl shadow-md border-l-4 hover:shadow-xl transition-all duration-300 ${salaryRemaining < 0 ? 'border-redError' : 'border-accent'
              }`}>
              <div className="flex items-center space-x-3 mb-3">
                <div className={`p-2 rounded-lg ${salaryRemaining < 0 ? 'bg-redError/10 text-redError' : 'bg-accent/10 text-accent'
                  }`}>
                  <TrendingUp size={22} />
                </div>
                <div>
                  <h4 className="text-sm font-medium text-mediumGray dark:text-gray-400">Wadarta Hadhay</h4>
                </div>
              </div>
              <p className={`text-2xl font-extrabold ${employee.totalRemaining < 0 ? 'text-redError' : 'text-accent'}`}>
                {employee.totalRemaining < 0 ? `-${Math.abs(employee.totalRemaining).toLocaleString()}` : employee.totalRemaining?.toLocaleString() || 0} ETB
              </p>
              <p className="text-xs text-mediumGray dark:text-gray-400 mt-1">
                {employee.totalRemaining < 0 ? 'Overpaid (Dayn)' : 'Weli u Baahan Bixin'}
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-4 md:p-5 rounded-xl shadow-md border-l-4 border-accent hover:shadow-xl transition-all duration-300">
              ...
              <p className="text-2xl font-extrabold text-accent">{employee.totalMonthsWorked || 0}</p>
              <p className="text-xs text-mediumGray dark:text-gray-400 mt-1">
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl p-5 md:p-6 border-l-4 border-primary mb-6 shadow-md">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                      <div className="bg-lightGray dark:bg-gray-700 p-3 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-primary">{employee.totalMonthsWorked || 0}</p>
                        <p className="text-xs text-mediumGray dark:text-gray-400 mt-1">
                          {new Date(employee.startDate).toLocaleDateString()}
                        </p>
                      </div>
                      ...
                      <div className="bg-lightGray dark:bg-gray-700 p-3 rounded-lg">
                        ...
                        <p className="text-lg font-bold text-primary">{employee.totalSalaryOwed?.toLocaleString() || 0} ETB</p>
                      </div>
                      <div className="bg-lightGray dark:bg-gray-700 p-3 rounded-lg">
                        <p className="text-xs text-mediumGray dark:text-gray-400 mb-1">Wadarta La Bixiyay</p>
                        <p className="text-lg font-bold text-secondary">{employee.totalPaid?.toLocaleString() || 0} ETB</p>
                        <p className="text-xs text-mediumGray dark:text-gray-400 mt-1">From All Salary Expenses</p>
                      </div>
                    </div>
                    <div className="bg-lightGray dark:bg-gray-700 p-4 rounded-lg border-l-4 border-accent">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <span className={`text-xl font-bold ${employee.totalRemaining < 0 ? 'text-redError' : 'text-secondary'}`}>
                          {employee.totalRemaining < 0 ? `-${Math.abs(employee.totalRemaining).toLocaleString()} ETB (Overpaid)` : `${employee.totalRemaining?.toLocaleString() || 0} ETB`}
                        </span>
                      </div>
                            ...
                            <div className="grid grid-cols-2 gap-1.5">
                              ...
                              <div className="bg-white dark:bg-gray-800 p-2 rounded-md shadow-sm border-l-2 border-primary hover:shadow transition-all">
                                ...
                                <div className="mt-1 w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1">
                                  <div
                                    className={`h-1 rounded-full transition-all duration-500 ${attendancePercentage >= 80 ? 'bg-green-500' :
                        ...
                        <div className="w-full overflow-x-auto">
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <div className={`flex flex-col items-center px-3 py-1.5 rounded-md transition-colors ${hasWorkedDays ? 'bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800' : ''
                                  }`}>
                                  <h3 className={`text-sm md:text-base font-semibold ${hasWorkedDays
                                  ...
                                  {employee?.startDate && (() => {
                                    ...
                                    return (
                                      <p className={`text-[10px] mt-0.5 ${hasWorkedDays
                          ...
                          <div className="flex flex-wrap gap-1.5 mb-2 p-2 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                            <button
                              ...
                              className={`px-2.5 py-1 rounded text-[11px] font-medium transition-all ${selectionMode === 'worked'
                            ...
                            <button
                              ...
                              className={`px-2.5 py-1 rounded text-[11px] font-medium transition-all ${selectionMode === 'notWorked'
                              ...
                              return days.map((date, index) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`aspect-square rounded border ${bgColor} ${textColor} ${borderColor} hover:scale-105 hover:shadow transition-all duration-150 flex items-center justify-center font-medium text-[10px] cursor-pointer ${isToday ? 'ring-1 ring-blue-400' : ''
                                      }`}
                                    title={dateStr}
                                  >
