...
import {
  ...
  Eye, Edit, Trash2, Loader2, Info as InfoIcon, CheckCircle, XCircle, Plus, Tag as TagIcon, ShoppingCart, Package
} from 'lucide-react';
      ...
      <div className="pb-20 md:pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <p className="text-2xl font-bold text-darkGray dark:text-white">Br{vendor.summary?.totalPurchases.toLocaleString()}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-sm text-gray-500 mb-1">Total Paid</p>
            <p className="text-2xl font-bold text-green-600">Br{vendor.summary?.totalPaid.toLocaleString()}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-sm text-gray-500 mb-1">Balance Due</p>
            <p className="text-2xl font-bold text-redError">Br{vendor.summary?.totalUnpaid.toLocaleString()}</p>
          </div>
          <div className="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <p className="text-sm text-gray-500 mb-1">Active Projects</p>
            <p className="text-2xl font-bold text-primary">{vendor.summary?.projects.length}</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden min-h-[500px]">
          <div className="border-b border-lightGray dark:border-gray-700 px-6">
            <nav className="flex space-x-8">
              {['Overview', 'Purchase Orders', 'Material History', 'Projects'].map(tab => (
                <button
                  key={tab}
                        ...
                        {vendor.purchaseOrders.map(po => (
                          <tr key={po.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <td className="px-4 py-3 text-gray-600 dark:text-gray-300">
                              {po.project ? <Link href={`/projects/${po.project.id}`} className="hover:underline">{po.project.name}</Link> : '-'}
                            </td>
