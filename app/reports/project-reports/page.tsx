...
async function exportPDF(data: ProjectReportsData, showDetails: boolean) {
  ...
  const renderDocument = (logoDataUrl?: string) => {
    ...
    const dateRange = data.startDate && data.endDate
      ? `${new Date(data.startDate).toLocaleDateString('en-GB')} - ${new Date(data.endDate).toLocaleDateString('en-GB')}`
    ...
    if (logoDataUrl) {
      try {
        // Simple fixed size for logo to avoid complex async structure in synchronous render
        // Or if we want aspect ratio, we can assume square or 4:3 default
        doc.addImage(logoDataUrl, 'PNG', 15, 8, 25, 15, undefined, 'FAST');
      } catch {
        // Fallback Logo
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);
    doc.text(companyName.toUpperCase(), 282, 15, { align: 'right' });
    ...
    drawCard(15, 'DAKHLIGA', `${formatCurrency(data.summary.totalRevenue)}`, 'ETB', [16, 185, 129]); // Emerald
    drawCard(85, 'KHARASHYADA', `${formatCurrency(data.summary.totalExpenses)}`, 'ETB', [239, 68, 68]); // Red
    drawCard(155, 'FAA\'IIDADA', `${formatCurrency(data.summary.totalProfit)}`, 'ETB', [59, 130, 246]); // Blue
    drawCard(225, 'MARGIN', `${data.summary.averageProfitMargin.toFixed(2)}%`, 'Average', [99, 102, 241]); // Indigo
    ...
    const projectHeaders = [['Mashruuc', 'Macmiil', 'Xaalad', 'Qiimaha', 'Dakhliga', 'Kharashyada', 'Faa\'iidada', '%']];
    ...
    if (showDetails && data.projects.length > 0) {
      data.projects.forEach((project) => {
        ...
        if (project.expenses.length > 0) {
          ...
          doc.setTextColor(30, 41, 59);
          doc.setFont('helvetica', 'bold');
          doc.text('Kharashyada', 15, detailY);
          detailY += 4;
          ...
          autoTable(doc, {
            startY: detailY,
            head: [['Taariikh', 'Nooca', 'Sharaxaad', 'Qiimaha']],
            body: expenseRows,
            theme: 'grid',
        ...
        // 2. Payments Table (Combined with Income transactions)
