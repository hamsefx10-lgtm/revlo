// app/reports/cashbook/page.tsx - Cashbook Report Page (API-driven)
'use client';
  ...
  useEffect(() => {
    async function fetchCashbook() {
      ...
      try {
        ...
        setError(err.message || 'Error fetching cashbook data');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) return (
    <Layout>
      <div className="flex items-center justify-center min-h-[400px] text-darkGray dark:text-gray-100">
        <span className="animate-spin mr-3"><DollarSign size={32} className="text-primary" /></span> Warbixinta Cashbook-ga ayaa soo dhacaya...
      </div>
    </Layout>
  );
  if (error) return (
