...
export default function LoginPage() {
  ...
  useEffect(() => {
    setMounted(true);
    const savedEmail = localStorage.getItem('revlo_remembered_email');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.ok) {
        if (rememberMe) localStorage.setItem('revlo_remembered_email', email.trim());
        else localStorage.removeItem('revlo_remembered_email');
        const response = await fetch('/api/auth/session');
        const session = await response.json();
        addNotification({ type: 'success', message: 'Si guul leh ayaad u soo gashay!' });
        setTimeout(() => {
          router.push(session?.user?.planType === 'SHOPS_ONLY' ? '/shop/dashboard' : '/dashboard');
        }, 500);
      } else {
      ...
      <div className={`w-full lg:w-1/2 xl:w-[45%] min-h-screen flex flex-col justify-center bg-white dark:bg-gray-950 relative z-10 transition-opacity duration-700 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
        <div className="w-full max-w-lg mx-auto px-8 sm:px-12 lg:px-16 py-12">
          ...
          <p className="mt-3 mb-12 text-base text-gray-400 dark:text-gray-500">
            Gali xogtaada si aad u sii wadato howlahaaga
          </p>
          ...
          <p className="mt-12 text-sm text-gray-400 dark:text-gray-500">
            Wali ma lihid akoon?{' '}
            <Link href="/signup" className="font-bold text-secondary hover:text-green-600 dark:hover:text-green-400 transition-colors">
              Is Diwaangali Hadda
            </Link>
          </p>
        </div>
