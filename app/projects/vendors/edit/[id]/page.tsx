...
export default function EditVendorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/projects/vendors/${id}`, { // Use PUT method for update
        method: 'PUT',
      ...
      if (response.ok) {
        ...
        router.push(`/projects/vendors/${id}`); // Redirect to vendor details page after update
      } else {
    ...
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold text-darkGray dark:text-gray-100">
          <Link href={`/projects/vendors/${id}`} className="text-mediumGray dark:text-gray-400 hover:text-primary transition-colors duration-200 mr-4">
