...
const CustomerRow: React.FC<{ customer: Customer; onEdit: (id: string) => void; onDelete: (id: string) => void }> = ({ customer, onEdit, onDelete }) => (
  <tr className="hover:bg-lightGray dark:hover:bg-gray-700 transition-colors duration-150 border-b border-lightGray dark:border-gray-700 last:border-b-0">
    ...
    <td className="p-4 whitespace-nowrap text-mediumGray dark:text-gray-300">
      <span className={`px-2 py-1 rounded-full text-xs font-semibold ${customer.type === 'Individual' ? 'bg-primary/10 text-primary' : 'bg-secondary/10 text-secondary'
        }`}>
        {customer.type}
      </span>
    </td>
    ...
    <td className="p-4 whitespace-nowrap text-right">
      <div className="flex items-center justify-end space-x-2">
        ...
        <Link href={`/projects/customers/edit/${customer.id}`} className="p-2 rounded-full bg-accent/10 text-accent hover:bg-accent hover:text-white transition-colors duration-200" title="Edit Customer">
          <Edit size={18} />
        </Link>
...
export default function CustomersPage() {
  ...
  const handleEditCustomer = (id: string) => {
    router.push(`/projects/customers/edit/${id}`); // Navigate to edit page
  };
    ...
    <Layout>
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex space-x-3">
          <Link href="/projects/customers/add" className="bg-primary text-white py-2.5 px-6 rounded-lg font-bold text-lg hover:bg-blue-700 transition duration-200 shadow-md flex items-center">
