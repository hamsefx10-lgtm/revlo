...
import {
  ...
  User as UserIcon, ReceiptText, Repeat, Download // Icons for various details and actions
} from 'lucide-react';
...
function Page() {
  ...
  const exportToExcel = () => {
    if (!account || account.transactions.length === 0) {
      setToastMessage({ id: Date.now().toString(), message: 'Ma jiraan dhaqdhaqaaqyo la dhoofin karo.', type: 'info' });
      return;
    }

    const dataToExport = account.transactions.map((trx) => {
      const isIncome = trx.isIncome;
      return {
        'Taariikhda': new Date(trx.transactionDate).toLocaleDateString(),
        'Sharaxaad': trx.description,
        'Nooca': trx.type,
        'Category': trx.category || 'N/A',
        'La Xiriira': trx.project?.name || trx.customer?.name || trx.vendor?.name || trx.employee?.fullName || trx.user?.fullName || 'N/A',
        'Soo Galay (In)': isIncome ? Math.abs(trx.amount) : 0,
        'Baxay (Out)': !isIncome ? Math.abs(trx.amount) : 0,
        'Haraaga (Bal)': trx.runningBalance,
      };
    });
    ...
    XLSX.writeFile(workbook, `${account.name}_Transactions_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
                    ...
                    <table className="min-w-full w-full divide-y divide-lightGray dark:divide-gray-700 bg-white dark:bg-gray-800 rounded-lg">
                      ...
                      <tbody className="divide-y divide-lightGray dark:divide-gray-700">
                        {account.transactions.map((trx: any) => {
                          ...
                          const runningBalance = trx.runningBalance;
                          ...
                          return (
                            <tr key={trx.id} className="hover:bg-lightGray dark:hover:bg-gray-700 transition-colors duration-150">
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-right font-semibold text-secondary">
                                {isIncome ? `$${amount.toLocaleString()}` : '-'}
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap text-right font-semibold text-redError">
                                {!isIncome ? `$${amount.toLocaleString()}` : '-'}
                              </td>
                              <td className={`px-4 py-3 whitespace-nowrap text-right font-bold border-l-2 border-lightGray dark:border-gray-600 bg-gray-50 dark:bg-gray-800/50 ${runningBalance >= 0 ? 'text-darkGray dark:text-gray-100' : 'text-redError'}`}>
                    ...
                    {account.transactions.map((trx) => {
                      ...
                      return (
                        <div key={trx.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-lightGray dark:border-gray-700 p-4 w-full">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm border-t border-lightGray dark:border-gray-700 pt-2 mb-2">
                            <div className="flex flex-col">
                              <span className="text-xs text-mediumGray">Soo Galay</span>
                              <span className="font-bold text-secondary">{isIncome ? `+$${Math.abs(trx.amount).toLocaleString()}` : '-'}</span>
                            </div>
                            <div className="flex flex-col text-right">
                              <span className="text-xs text-mediumGray">Baxay</span>
                              <span className="font-bold text-redError">{!isIncome ? `-$${Math.abs(trx.amount).toLocaleString()}` : '-'}</span>
                            </div>
                            <div className="col-span-2 flex justify-between items-center bg-gray-50 dark:bg-gray-800/50 p-2 rounded border border-lightGray dark:border-gray-700">
                              <span className="text-xs font-semibold text-darkGray dark:text-gray-300">Haraaga Hadda:</span>
                              <span className={`font-bold ${trx.runningBalance >= 0 ? 'text-darkGray dark:text-gray-100' : 'text-redError'}`}>${trx.runningBalance.toLocaleString()}</span>
                            </div>
                          </div>
