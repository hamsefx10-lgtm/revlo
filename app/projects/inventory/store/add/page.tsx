...
export default function AddInventoryItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/projects/inventory/store', { // API endpoint for adding inventory items
        method: 'POST',
      ...
      if (response.ok) {
        ...
        router.push('/projects/inventory/store'); // Redirect to inventory list on success
      } else {
    ...
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold text-darkGray dark:text-gray-100">
          <Link href="/projects/inventory/store" className="text-mediumGray dark:text-gray-400 hover:text-primary transition-colors duration-200 mr-4">
