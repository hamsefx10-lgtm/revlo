...
export default function InventoryStorePage() {
  ...
  const handleDeleteItem = async (id: string) => {
    if (window.confirm('Ma hubtaa inaad tirtirto alaabtan? Tan lama soo celin karo!')) {
      try {
        const response = await fetch(`/api/projects/inventory/${id}`, {
          method: 'DELETE',
        });
        const data = await response.json();
  ...
  const handleEditItem = (id: string) => {
    router.push(`/projects/inventory/store/edit/${id}`); // Navigate to edit page
  };

  const handleRestockItem = (id: string) => {
    router.push(`/projects/inventory/store/restock/${id}`); // Navigate to restock page
  };
    ...
    <Layout>
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-6 gap-4">
        <div className="flex items-center gap-3 lg:gap-4">
          <Link href="/projects/inventory" className="text-mediumGray dark:text-gray-400 hover:text-primary transition-colors duration-200">
            <ArrowLeft size={20} className="inline-block" />
          </Link>
          <h1 className="text-2xl lg:text-4xl font-bold text-darkGray dark:text-gray-100">Inventory Report</h1>
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
          <Link href="/projects/inventory/store/add" className="bg-primary text-white py-2 lg:py-2.5 px-4 lg:px-6 rounded-lg font-bold text-sm lg:text-lg hover:bg-blue-700 transition duration-200 shadow-md flex items-center justify-center">
            <Plus size={18} className="mr-2" /> Ku Dar Alaab
          </Link>
          <button onClick={fetchInventoryItems} className="bg-secondary text-white py-2 lg:py-2.5 px-4 lg:px-6 rounded-lg font-bold text-sm lg:text-lg hover:bg-green-600 transition duration-200 shadow-md flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8 animate-fade-in-up">
        <div className="bg-white dark:bg-gray-800 p-4 lg:p-6 rounded-xl shadow-md text-center">
          <h4 className="text-sm lg:text-lg font-semibold text-mediumGray dark:text-gray-400">Wadarta Alaabta</h4>
          <p className="text-2xl lg:text-3xl font-extrabold text-primary">{totalItemsCount}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 lg:p-6 rounded-xl shadow-md text-center">
          <h4 className="text-sm lg:text-lg font-semibold text-mediumGray dark:text-gray-400">Wadarta Quantity Stock-ga</h4>
          <p className="text-2xl lg:text-3xl font-extrabold text-secondary">{totalInStockQty}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 lg:p-6 rounded-xl shadow-md text-center">
          <h4 className="text-sm lg:text-lg font-semibold text-mediumGray dark:text-gray-400">Alaab Yar</h4>
          <p className="text-2xl lg:text-3xl font-extrabold text-redError">{lowStockItemsCount}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 lg:p-6 rounded-xl shadow-md text-center">
          <h4 className="text-sm lg:text-lg font-semibold text-mediumGray dark:text-gray-400">Wadarta Qiimaha (Cost)</h4>
          <p className="text-2xl lg:text-3xl font-extrabold text-accent">${totalValueAtCost.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-4 lg:p-6 rounded-xl shadow-md mb-6 lg:mb-8 flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 animate-fade-in-up">
        ...
        <div className="relative w-full md:w-48">
          <Warehouse size={18} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-mediumGray dark:text-gray-400" />
          <select
            className="w-full p-2 lg:p-3 pl-9 lg:pl-10 border border-lightGray dark:border-gray-700 rounded-lg bg-lightGray dark:bg-gray-700 text-darkGray dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition duration-200 appearance-none text-sm lg:text-base"
