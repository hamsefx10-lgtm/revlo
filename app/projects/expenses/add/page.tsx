...
function AddExpenseContent() {
  ...
  const fetchProjectLabors = async () => {
    try {
      const res = await fetch('/api/projects/expenses/project-labors');
      if (res.ok) {
  ...
  const fetchCompanyLabors = async () => {
    try {
      const res = await fetch('/api/projects/expenses/company-labors');
      if (res.ok) {
        const data = await res.json();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    switch (categoryForSwitch) {
      case 'Material':
        ...
        if (expenseType === 'company') {
          ...
          // Don't force category='Material' here, it's already set correctly
          if (companyExpenseType === 'Material') {
            expenseData.category = 'Material';
            expenseData.expenseDate = materialDate;
            expenseData.paidFrom = (paymentStatus === 'PAID' || paymentStatus === 'PARTIAL') ? paidFrom : null;
          } else {
            // For non-material company expenses, ensure category is 'Company Expense'
      ...
      case 'Labor':
        ...
        // Payment Status logic for Project Labor
        if (paidFrom && paidFrom !== 'UNPAID') {
      ...
      case 'Company Expense':
        ...
        switch (companyExpenseType) {
          case 'Salary':
            ...
            // Logic for salary payment
            if (salaryPaymentAmount && paidFrom) {
              expenseData.paymentStatus = 'PAID';
              expenseData.paymentDate = salaryPaymentDate || new Date().toISOString();
              expenseData.amount = salaryPaymentAmount; // Override amount with actual payment
            } else {
              expenseData.paymentStatus = 'UNPAID';
            }
          ...
          case 'Company Labor':
            ...
            expenseData.subCategory = 'Company Labor';
            expenseData.employeeId = selectedEmployeeForSalary;
            expenseData.agreedWage = wage;
            expenseData.laborPaidAmount = laborPaidAmount;
            if (laborPaidAmount && paidFrom) {
              expenseData.paymentStatus = 'PAID';
              expenseData.paymentDate = expenseDate || new Date().toISOString();
              expenseData.amount = laborPaidAmount;
            } else {
              expenseData.paymentStatus = 'UNPAID';
            }
            expenseData.description = workDescription;
    ...
    // Universal Payment Status Logic
    // If paymentStatus is not explicitly set (or is UNPAID/undefined), derive it from 'paidFrom'
    // This ensures that for categories like Transport, Labor, etc., if a payment account is selected, it's marked as PAID.
    if (!expenseData.paymentStatus || expenseData.paymentStatus === 'UNPAID') {
      if (expenseData.paidFrom) {
        expenseData.paymentStatus = 'PAID';
        expenseData.paymentDate = expenseData.paymentDate || expenseData.expenseDate || new Date().toISOString();
        if (expenseData.paidAmount === undefined) {
          expenseData.paidAmount = expenseData.amount;
        }
      } else {
        // explicit UNPAID if no paidFrom, though usually validation blocks this for most types
        expenseData.paymentStatus = 'UNPAID';
      }
    }
    ...
    try {
      ...
      if (category === 'Labor' && expenseType === 'project') {
        const laborPayload = {
          ...
          paymentStatus: expenseData.paymentStatus, // NEW: Include payment status
          paymentDate: expenseData.paymentDate, // NEW: Include payment date
        ...
        const expenseRes = await submitExpenseData('/api/projects/expenses/project', laborPayload);
      ...
      if (expenseType === 'company' && companyExpenseType === 'Company Labor') {
        ...
        const expenseRes = await submitExpenseData('/api/projects/expenses/company', companyLaborPayload);
        const expenseDataRes = await expenseRes.json();
        if (!expenseRes.ok) {
          const errorMsg = expenseDataRes.message || (typeof expenseDataRes === 'string' ? expenseDataRes : JSON.stringify(expenseDataRes)) || 'Failed to record expense';
          console.error('Backend error response (expense):', expenseDataRes);
      ...
      const endpoint = expenseType === 'project' ? '/api/projects/expenses/project' : '/api/projects/expenses/company';
    ...
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold text-darkGray dark:text-gray-100">
          <Link href="/projects/expenses" className="text-mediumGray dark:text-gray-400 hover:text-primary transition-colors duration-200 mr-4">
