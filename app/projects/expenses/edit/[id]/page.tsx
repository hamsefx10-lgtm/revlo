...
export default function EditExpensePage() {
  ...
  useEffect(() => {
    if (id) {
      const fetchExpense = async () => {
        setPageLoading(true);
        try {
          const res = await fetch(`/api/projects/expenses/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Universal Payment Status Logic (matches Add page)
    let finalPaymentStatus = paymentStatus;
    if (paidFrom && (!paymentStatus || paymentStatus === 'UNPAID')) {
    ...
    let expenseData: any = {
      ...
      amount: (category === 'Material' || companyExpenseType === 'Material') ? totalMaterialCost : (category === 'Company Labor' || companyExpenseType === 'Company Labor' ? laborPaidAmount : amount),
      materials: (category === 'Material' || companyExpenseType === 'Material') ? materials.map(({ id, ...rest }) => rest) : undefined,
      materialDate: (category === 'Material' || companyExpenseType === 'Material') ? materialDate : undefined,
      ...
      // Material / Shared fields
      selectedVendor: (companyExpenseType === 'Material' || category === 'Material') ? selectedVendor : undefined,
    ...
    try {
      const response = await fetch(`/api/projects/expenses/${id}`, {
        method: 'PUT',
      ...
      router.push(`/projects/expenses/${id}`);
    ...
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold text-darkGray dark:text-gray-100 flex items-center">
          <Link href={`/projects/expenses/${id}`} className="p-2 rounded-full bg-lightGray dark:bg-gray-700 hover:bg-primary hover:text-white transition-colors duration-200 mr-4">
            <ArrowLeft size={28} />
          </Link>
