...
export default function ShopDashboard() {
    ...
    const [stats, setStats] = useState({
        metrics: { revenue: 0, orders: 0, products: 0, lowStock: 0 },
        chartData: [] as any[],
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <MetricCard
                            label="Total Revenue"
                            value={`ETB ${stats.metrics.revenue.toLocaleString()}`} // formatting
                            trend="Live"
                            isPositive={true}
                            icon={Wallet}
                            variant="primary"
                        ...
                        <MetricCard
                            ...
                            trend={stats.metrics.lowStock > 0 ? "Action needed" : "Good"}
                            ...
                            <div className="bg-white dark:bg-[#0f172a] rounded-[20px] p-8 h-[460px] flex flex-col shadow-sm dark:shadow-none">
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500 dark:text-gray-400">Revenue in ETB (Last 7 Days)</p>
                                    </div>
                                </div>
                            ...
                            <div className="bg-white/50 dark:bg-[#1f2937]/30 backdrop-blur-md rounded-[24px] p-1 border border-gray-100 dark:border-white/5 shadow-sm">
                                <div className="bg-white dark:bg-[#0f172a] rounded-[20px] p-6 shadow-sm dark:shadow-none">
                                    <div className="flex items-center justify-between mb-4">
                                        <h4 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Pending Payments</h4>
                                        <Link href="/shop/purchases" className="text-xs font-bold text-[#3498DB] hover:text-[#2980B9]">View All</Link>
                                    </div>
                                    <div className="space-y-3">
                                        {!stats.unpaidOrders || stats.unpaidOrders.length === 0 ? (
                                            <p className="text-sm text-gray-500 text-center py-4">No pending payments</p>
                                        ) : (
                                            stats.unpaidOrders.map((po: any) => (
                                                <div key={po.id} className="flex flex-col gap-3 p-4 rounded-2xl bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30">
                                                    <div className="flex justify-between items-start">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <p className="text-sm font-bold text-gray-900 dark:text-white leading-tight">{po.vendor?.companyName || 'Supplier'}</p>
                                                                <p className="text-[10px] text-red-500 font-medium uppercase tracking-wide mt-0.5">{po.poNumber}</p>
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <div className="flex items-center justify-between pl-11">
                                                        <div className="text-xs font-medium text-gray-500">Due Amount</div>
                                                        <div className="text-sm font-black text-gray-900 dark:text-white">ETB {(po.total - po.paidAmount).toLocaleString()}</div>
                                                    </div>

                                                    <Link
                                                        href={`/shop/purchases/${po.id}/pay`}
                                                        className="w-full py-2.5 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-900 dark:text-white rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all shadow-sm border border-gray-200 dark:border-gray-700 group"
                                                    >
                                                        Pay Now <ChevronRight size={14} className="text-[#3498DB] group-hover:translate-x-1 transition-transform" />
                                                    </Link>
                                                </div>
                                            ))
                                        )}
                            ...
                            <div className="bg-white/50 dark:bg-[#1f2937]/30 backdrop-blur-md rounded-[24px] p-1 border border-gray-100 dark:border-white/5 shadow-sm">
                                <div className="bg-white dark:bg-[#0f172a] rounded-[20px] p-6 shadow-sm dark:shadow-none">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <Link href="/shop/inventory/add" className="flex flex-col items-center justify-center p-4 rounded-2xl bg-gray-50 dark:bg-gray-800 border-2 border-dashed border-gray-200 dark:border-gray-700 hover:border-[#F39C12] hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-all group cursor-pointer text-center">
                            ...
                            <div className="bg-white/50 dark:bg-[#1f2937]/30 backdrop-blur-md rounded-[24px] p-1 border border-gray-100 dark:border-white/5 shadow-sm">
                                <div className="bg-white dark:bg-[#0f172a] rounded-[20px] p-6 shadow-sm dark:shadow-none">
                                    <div className="flex items-center justify-between mb-4">
                                        ...
                                        <Link href="/shop/inventory?filter=Low%20Stock" className="text-xs font-bold text-[#3498DB] hover:text-[#2980B9]">See All</Link>
                                    </div>
                                    <div className="space-y-3">
                                        {stats.lowStockItems.length === 0 ? (
                                            ...
                                            stats.lowStockItems.map((item: any, i: number) => (
                                                <div key={item.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors cursor-pointer">
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-xs text-red-500 font-medium">{item.stock} units left</p>
                                                    </div>
