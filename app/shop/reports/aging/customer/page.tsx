...
export default function CustomerAgingReport() {
    ...
    useEffect(() => {
        fetch('/api/shop/reports/aging/customer')
            .then(res => res.json())
            .then(setData)
            ...
            {loading ? <Loader2 className="animate-spin text-blue-500 mx-auto" /> : (
                <div className="space-y-8">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-8 shadow-sm border border-gray-100 dark:border-gray-700">
                        <h3 className="font-bold text-lg mb-6">Aged Receivables Detail</h3>
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                                {data.details.map((row: any) => (
                                    <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="py-4 font-bold">{row.customer}</td>
                                        <td className="py-4 font-mono text-sm text-gray-500">{row.invoiceNumber}</td>
